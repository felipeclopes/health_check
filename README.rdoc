...
To change the configuration of health_check, create a file `config/initializers/health_check.rb` and add a configuration block like:
    ...
    HealthCheck.setup do |config|
      ...
      # http status code used when an error object is output (json, jsonp or xml)
...
=== Requesting JSONP responses

Health_check will respond with a JSONP response (content-type: application/javascript)
where the url ends in `?callback=name_of_callback_function`.
The response will be prepended with a comment and a test
that the callback argument is a valid function to aid client side security.

curl -v localhost:3000/health_check.json?callback=my_function

will return:

  /**/ my_function({"healthy":true,"message":"success"});

Note: the callback value must be single javascript identfier that is not a reserved word and contains only ascii characters.
